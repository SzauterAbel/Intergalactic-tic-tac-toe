...
from PyQt5.QtWidgets import (QMainWindow, QWidget, QGridLayout, QVBoxLayout, 
                             QHBoxLayout, QPushButton, QLabel, QMessageBox, QFrame, QScrollArea, QLineEdit, QStackedWidget, QFileDialog)
...
class CellButton(QPushButton):
    ...
    def highlight_active(self):
        """Highlight this cell as part of the active block."""
        self.setStyleSheet("""
            QPushButton {
...
class Block9x9Frame(QFrame):
    ...
    def get_cell_button(self, row, col):
        ...
        local_row = row % 3
        local_col = col % 3
        return self.blocks_3x3[block_3x3_row][block_3x3_col].get_button(local_row, local_col)
    ...
    def _on_load_clicked(self):
        ...
        if dialog.exec_():
            ...
            try:
                ...
                QMessageBox.critical(self, 'Load Failed', f'Could not load game: {str(e)}')
...
class GameBoard(QWidget):
    ...
    def get_cell_button(self, row, col):
        ...
        local_row = row % 9
        local_col = col % 9
        return self.blocks_9x9[block_9x9_row][block_9x9_col].get_cell_button(local_row, local_col)
    ...
    def update_board(self):
        ...
        if self.game.active_3x3_block is not None and self.game.active_9x9_block is not None:
            ...
            for row in range(27):
                for col in range(27):
                    ...
                    if btn.player == '':
                        ...
                        if (self.game.blocks_3x3[block_3x3_row][block_3x3_col] == '' and
                            self.game.blocks_9x9[block_9x9_row][block_9x9_col] == ''):
                            btn.highlight_active()
        ...
        for block_row in range(9):
            for block_col in range(9):
                ...
                if winner:
                    ...
                    self.blocks_9x9[block_9x9_row][block_9x9_col].blocks_3x3[block_3x3_row][block_3x3_col].highlight_won(winner)
    ...
    def reset_board(self):
        ...
        for block_9x9_row in range(3):
            for block_9x9_col in range(3):
                ...
                for block_3x3_row in range(3):
                    for block_3x3_col in range(3):
                        self.blocks_9x9[block_9x9_row][block_9x9_col].blocks_3x3[block_3x3_row][block_3x3_col].highlight_won('')
...
class GamePanel(QWidget):
    ...
    def __init__(self, player_x_name: str, player_o_name: str):
        ...
        # Status label
        self.status_label = QLabel(f"Game Started - {player_x_name} (X) plays first (can play anywhere)")
            ...
            for block_9x9_col in range(3):
                for block_3x3_row in range(3):
                    for block_3x3_col in range(3):
                        ...
                        for local_row in range(3):
                            for local_col in range(3):
                                btn = self.board.blocks_9x9[block_9x9_row][block_9x9_col].blocks_3x3[block_3x3_row][block_3x3_col].get_button(local_row, local_col)
    ...
    def update_display(self):
        ...
        if self.game.status == 'playing':
            player_label = self.player_names.get(self.game.current_player, self.game.current_player)
    ...
    def _on_save_game(self):
        ...
        try:
            filepath = GamePersistence.save_game(self.game, self.player_names['X'], self.player_names['O'])
            filename = os.path.basename(filepath)
            QMessageBox.information(self, 'Save Successful', f'Game saved as: {filename}')
        except Exception as e:
    ...
    def _on_load_game(self):
        ...
        if dialog.exec_():
            ...
            try:
                ...
                self.update_display()
                QMessageBox.information(self, 'Load Successful', f'Game loaded: {os.path.basename(filepath)}')
            except Exception as e:
                QMessageBox.critical(self, 'Load Failed', f'Could not load game: {str(e)}')
